...
//checking for user input for student sign up
if (isset($_POST['register'])) {
    ...
    if (empty($name) || empty($username) || empty($password) || empty($confirm)) {
        ...
        if ($password === $confirm) {
            ...
            foreach (login_student() as $student) {
                if ($student['username'] == $username) {
                    $check_registered = TRUE;
                }
            }
            if (!$check_registered) {
                $register = register_student($name, $username, $password);
                if ($register) {
                    $message = '<div style="color: green;">**Registration successful**
                       <a href="student_login.php">Click Here </a> to login.</div>';
                }
            } else {
